...
//           </Box>
//           {/* -------------------- */}
//           <Button variant="contained" fullWidth onClick={handleUpdateTravelClick}
...
function EditMyTravel() {
  ...
  useEffect(() => {
    const travellerRaw = localStorage.getItem('traveller')
    if (!travellerRaw) {
      alert("กรุณาเข้าสู่ระบบก่อน");
      navigator('/');
      return;
    }
    const traveller = JSON.parse(travellerRaw)
    setTravellerFullname(traveller.travellerFullname)
    setTravellerImage(traveller.travellerImage)
    setTravellerId(traveller.travellerId)
    ...
    const getTravel = async () => {
      try {
        const resData = await axios.get(`https://travel-serveice-by-prisma.vercel.app/travel/one/${travelId}`)
        const travel = resData.data?.data
        if (travel) {
  ...
  const handleSelectFileClick = (e) => {
    const file = e.target.files[0]
    if (file) setTravelNewImage(file)
  }
  ...
  const handleUpdateTravelClick = async (e) => {
    ...
    if (travelPlace.trim() === '') {
      alert('ป้อนสถานที่ไปด้วย')
    } else if (travelStartDate.trim() === '') {
      alert('ป้อนวันที่ไปด้วย')
    } else if (travelEndDate.trim() === '') {
      alert('ป้อนวันที่กลับด้วย')
    } else if (travelCostTotal.length === 0) {
      ...
      formData.append('travelCostTotal', travelCostTotal)
      formData.append('travellerId', travellerId)
      if (travelNewImage) formData.append('travelImage', travelNewImage)

      try {
        // บาง backend ไม่รองรับ PUT+multipart, ลองใช้ POST แทน
        ...
        if (response.status === 200) {
          ...
          alert('แก้ไขการเดินทางไม่สำเร็จ กรุณาลองใหม่อีกครั้ง')
        }

      } catch (error) {
        console.error("เกิดข้อผิดพลาด", error)
        alert('พบข้อผิดพลาดในการทำงาน: ' + (error.response?.data?.message || error.message))
      }
    }
  }

  const displayImage = travelNewImage
    ? URL.createObjectURL(travelNewImage)
    : travelImage
    ? `https://res.cloudinary.com/dohai2o2w/image/upload/v1/images/travel/${travelImage}`
    : Place
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <Box sx={{ width: '60%', boxShadow: 4, mx: 'auto', p: 5, my: 4 }}>
        ...
        <Avatar src={Travel} sx={{ width: 150, height: 150, mx: 'auto', my: 2 }} />
        <Typography variant='h5' sx={{ textAlign: 'center', fontWeight: 'bold' }}>
          เพิ่มการเดินทาง
        </Typography>

        <Typography sx={{ fontWeight: 'bold', mt: 4, mb: 1 }}>สถานที่ที่เดินทางไป</Typography>
        <TextField fullWidth value={travelPlace} onChange={(e) => setTravelPlace(e.target.value)} />

        <Typography sx={{ fontWeight: 'bold', mt: 4, mb: 1 }}>วันที่เดินทางไป</Typography>
        <TextField fullWidth value={travelStartDate} onChange={(e) => setTravelStartDate(e.target.value)} />

        <Typography sx={{ fontWeight: 'bold', mt: 4, mb: 1 }}>วันที่เดินทางกลับ</Typography>
        <TextField fullWidth value={travelEndDate} onChange={(e) => setTravelEndDate(e.target.value)} />

        <Typography sx={{ fontWeight: 'bold', mt: 4, mb: 1 }}>ค่าใช้จ่ายในการเดินทาง</Typography>
        <TextField fullWidth type='number' value={travelCostTotal} onChange={(e) => setTravelCostTotal(e.target.value)} />

        <Avatar src={displayImage} alt="travel" sx={{ width: 150, height: 150, mx: 'auto', my: 3 }} variant="rounded" />
